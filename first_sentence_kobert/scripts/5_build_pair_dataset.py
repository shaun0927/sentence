...
def main(args):
    ...
    per_doc = defaultdict(list)      # doc_id → [(idx , P_first)]
    for (doc, idx), p_first in zip(meta, prob[:, 1]):
        per_doc[doc].append((idx, float(p_first)))
    ...
    for doc_id, lst in per_doc.items():
        ...
        gold_first     = int(row_src["answer_0"])
        label          = int(idx_a == gold_first)
